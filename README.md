...
| (+15–16) | Curvature look-ahead ×2 | −1–1 | *(Phase-3 옵션 `lookahead`)* 30 m·70 m 앞 트랙이 꺾이는 각(`/π`, 부호=좌/우) — §6-2 |
...
Phase-2 최적(`timeattack`)도 성공률이 낮은 핵심 원인은 **코너**입니다(§9-7). 세 개의 직교 레버로 이를 공략합니다:
...
- **③ 곡률 look-ahead**(env `lookahead`, obs +2차원): 지금 obs는 *현재* 지점 heading과 raycast(벽 거리)뿐이라
  코너를 **반응적으로** 인지합니다. 30 m·70 m 앞 트랙이 꺾이는 각을 obs에 넣어(페이스 노트처럼) **미리** 감속/조향
  하도록 — raycast와 **직교**한 정보(벽 거리 vs 도로의 예정 모양). λ와 무관한 **on/off ablation**(§9-5 raycast와 동일 패턴)
  으로, 커리큘럼(λ=0) 위에 lookahead만 토글해 격리합니다.
